...
**Last session:** M29 — Crash Reporting, Analytics & Observability complete. Sentry (`sentry_flutter: ^8.9.0`): `SentryFlutter.init` wrapping `main()`, `SentryNavigatorObserver`, PII scrubbing (`user` stripped from every event via `beforeSend`), `tracesSampleRate: 0.2`, `captureException` in `auth_service` (10 methods), `gemini_ai_service` (6 locations + 2 transactions: `ai-workout-plan`, `ai-nutrition-plan`), `food_recognition_service` (`food-recognition` transaction), `ai_workout_generator` (4 catch blocks), `nutrition_planning_screen` (3 swallowed catches). PostHog (`posthog_flutter: ^4.8.0`): `AnalyticsService` singleton (`lib/services/analytics_service.dart`), `POSTHOG_API_KEY` from env.json, EU-hosted (`eu.posthog.com`), SharedPreferences-based opt-out (`analytics_opt_out` key), `trackFirstOnce()` helper with SharedPreferences flag guard, 7 funnel events: `signup_started`, `onboarding_completed`, `first_ai_plan_generated`, `first_workout_logged`, `first_meal_logged`, `photo_recipe_generated`, `barcode_scanned`. Analytics opt-out `SwitchListTile` in `SecuritySettingsScreen`. In-app review (`in_app_review: ^2.0.9`): fires in `_finishWorkout()` after 3+ workouts + 7-day signup age + `review_asked` prefs flag. `upgrader: ^11.0.0`: `UpgradeAlert` wraps `MaterialApp` for Play Store / App Store version checks. `flutter analyze lib/` → **0 issues**.
**Next session starts with:** M30 — Testing, CI & Quality Gate
**Active branches:** main
**Blockers:** `pubspec.lock` is gitignored — run `flutter pub get` at every session start. `kotlin.incremental=false` in android/gradle.properties — required for Windows cross-drive build (pub cache on C:, project on D:); do not remove. DB enum values fully English. USDA_API_KEY in env.json only. Gemini 2.5 Flash needs `maxTokens ≥ 8192` for OCR. Exercise animations use free-exercise-db CDN. `Stack(fit: StackFit.expand)` required for IndexedStack overlay. `memCacheWidth` needs `.isFinite` guard. M20 manual config: "Confirm email" ✅ enabled; hCaptcha skipped (PAAM project, no paid tier needed). M19 deferred: pagination UI, lazy ProgressTrackingScreen, SharedPreferences layer. `DropdownButtonFormField` uses `initialValue:` not deprecated `value:` (Flutter 3.33+). Supabase remaining (intentional/deferred): 2 food_database rls_policy_always_true (any auth user can add/edit foods — by design), unused_index INFO on indexes (new — no query traffic yet), auth_leaked_password_protection (enable manually: Supabase Dashboard → Auth → Password Security). **M29 manual setup required**: register at sentry.io → add `SENTRY_DSN` to `env.json`; register at posthog.com → add `POSTHOG_API_KEY` to `env.json`. PostHog 4.x has no native `optOut()`/`optIn()` — opt-out is SharedPreferences-only (`_isOptedOut` in `AnalyticsService`). `User.createdAt` in Supabase Flutter returns `String` — always use `DateTime.tryParse(user.createdAt)`.
