...
**M24 Image & Asset Optimization patterns** (2026-04-19):
- **WebP assets**: `assets/images/no-image.webp` (was .jpg) + `body_silhouette.webp` (was .png). `CustomImageWidget.placeHolder` default already updated — no call-site changes needed.
- **flutter_image_compress**: import `flutter_image_compress/flutter_image_compress.dart`; call `FlutterImageCompress.compressWithList(rawBytes, minWidth:1024, minHeight:1024, quality:75, format:CompressFormat.jpeg)` **before** passing bytes to any Gemini API call. For file-path flows (photo uploads), use `compressAndGetFile(sourcePath, targetPath, quality:80)` with `Directory.systemTemp.path` for the target directory.
- **memCacheHeight in Exercise3DWidget**: `CachedNetworkImage` in `exercise_3d_widget.dart` now carries `memCacheHeight: widget.height.isFinite && widget.height > 0 ? widget.height.toInt() : null` — same `.isFinite` guard as `CustomImageWidget`.
- **precacheImage warm-up**: After `_loadExercises()` populates `_filteredExercises`, loop `_filteredExercises.take(10)` and call `precacheImage(NetworkImage(url), context)` inside `if (mounted)` — warms first-page exercise frames before user scrolls.
- **Native splash**: generated by `flutter_native_splash`; green (`#2E7D32`) light / dark navy (`#1B3A5E`) dark; fullscreen centered logo. Source images: `assets/images/icon_source.png` (1024×1024) + `splash_logo.png` (300×300) — re-generate with `dart run flutter_native_splash:create` if splash theme changes.
- **kotlin.incremental=false** in `android/gradle.properties` — required on Windows when pub cache (C:) and project (D:) are on different drives; cross-drive relative paths crash the Kotlin daemon. Do not remove.
